...
class Chamado(models.Model):
    ...
    tipo = models.ForeignKey(TipoChamado, on_delete=models.SET_NULL, verbose_name='Grupo do chamado', null=True)
    subtipo = models.ForeignKey(SubTipoChamado, on_delete=models.SET_NULL, verbose_name='Serviços realizado', null=True)
    relatorio = models.TextField(verbose_name='Observação', null=True)
...
class chamadoSatisfacao(models.Model):
    ...
    def get_icon_avaliacao(self):
        valor = self.avaliacao
        return self.get_icon(valor)
    
    def get_icon_cordialidade(self):
        valor = self.cordialidade
        return self.get_icon(valor)
    
    def get_icon_tempo_espera(self):
        valor = self.tempo_espera
        return self.get_icon(valor)